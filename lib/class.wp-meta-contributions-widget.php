<?php
if ( ! class_exists('WP_Meta_Contributions_Widget') ) :

class WP_Meta_Contributions_Widget extends WP_Widget {
	function WP_Meta_Contributions_Widget() {
		$widget_ops = array(
			'classname' => 'widget_meta_contributions',
			'description' => __( 'Add a list of your accepted contributions to WordPress Meta as a sidebar widget.', 'wp-core-contributions-widget' )
		);

		$this->WP_Widget( false, __( 'WP Meta Contributions', 'wp-core-contributions-widget' ), $widget_ops );
	}
	...
	function form( $instance ) {
		...
		if ( $instance && isset( $instance[ 'title' ] ) ) {
			...
			$title = esc_attr__( 'WP Meta Contributions', 'wp-core-contributions-widget' );
		}
	...
	function widget( $args, $instance ){
		...
		// Widget content
		$items = array_slice( WP_Meta_Contributions::get_items( $user ), 0, $count );
		$total = WP_Meta_Contributions::get_changeset_count( $user );

		// Include template - can be overriden by a theme!
		$template_name = 'wp-meta-contributions-widget-template.php';
